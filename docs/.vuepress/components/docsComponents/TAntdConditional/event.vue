<template>
  <t-antd-layout-page>
    <t-antd-conditional :opts="conditionParams" @submit="conditionEnter" />
  </t-antd-layout-page>
</template>
<script>
...
export default {
    ...
    conditionParams() {
      return {
        originFurnaceNumber: {
          ...
          changeEvent: "change",
          event: {
            change: (val) => this.originFurnaceNumber(val)
          },
        ...
        dealedFurnaceNumber: {
          ...
          changeEvent: "change",
          event: {
            change: (val) => this.dealedFurnaceNumber(val)
          },
        ...
        dealedStatusLst: {
          ...
          span: 2,
          event: {
            change: (val) => this.dealedStatusLst(val)
          },
          bind: {
            mode: "multiple",
          },
  ...
  methods: {
    originFurnaceNumber(val) {
      console.log('源编号change事件', val)
    },
    dealedFurnaceNumber(val) {
      console.log('目标编号change事件', val)
    },
    dealedStatusLst(val) {
      console.log('处理状态change事件', val)
    },
    // 查询
    conditionEnter(obj) {

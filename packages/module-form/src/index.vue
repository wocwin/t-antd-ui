  ...
  >
    <t-antd-layout-page>
      <div class="scroll_wrap">
        ...
        <!-- 表单页面 -->
        <t-antd-form v-if="handleType==='edit'" v-bind="$attrs" ref="tAntdForm">
          <template v-for="(index, name) in $slots" :slot="name">
            <slot :name="name" />
          </template>
        </t-antd-form>
        <!-- 详情页面 -->
        <t-antd-detail v-else v-bind="$attrs">
          <template v-for="(index, name) in $slots" :slot="name">
            <slot :name="name" />
          </template>
        </t-antd-detail>
        <t-antd-layout-page-item class="tabs" v-if="tabs">
          <a-tabs
            v-if="tabs.length > 1"
...
<script>
import { PageHeader, Button, Drawer, Tabs } from 'ant-design-vue'
import TAntdDetail from './t-antd-detail'
import TAntdForm from './t-antd-form'
export default {
